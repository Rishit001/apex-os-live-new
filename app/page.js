...
function waitForStorage() {
  return new Promise(resolve => {
    if (typeof window !== "undefined" && window.storage) return resolve();
    let tries = 0;
    const t = setInterval(() => {
      // Sped up the fallback check so it doesn't hang
      if ((typeof window !== "undefined" && window.storage) || ++tries > 10) { clearInterval(t); resolve(); }
    }, 50);
  });
}
...
async function sGet(key, fallback) {
  try {
    ...
    if (typeof window !== "undefined" && window.storage) {
      ...
      // Standard browser fallback (This is what will save you)
...
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// API (Generic Omni-Router)
// ─────────────────────────────────────────────────────────────────────────────
async function claude(system, messages) {
  const apiKey = await sGet("llm_api_key", "");
  const baseUrl = await sGet("llm_base_url", "https://api.groq.com/openai/v1");
  const model = await sGet("llm_model", "llama-3.3-70b-versatile");

  if (!apiKey) throw new Error("No API key — please enter it in Settings or refresh the page.");

  // Clean the URL to ensure it ends correctly
  ...
  const res = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      model: model,
      max_tokens: 8000, // <-- THIS UNLOCKS THE MAX REPLY LIMIT
      messages: [
...
function Settings({ cfg, setCfg, notify }) {
  ...
  useEffect(() => {
    sGet("llm_api_key", "").then(setApiKey);
    sGet("llm_base_url", "https://api.groq.com/openai/v1").then(setBaseUrl);
...
// SOPs
// ─────────────────────────────────────────────────────────────────────────────
const SOP_PLACEHOLDERS = {
  s1: "e.g. Creating a Reel about replacing your team with AI. Filming tomorrow, post by Friday.",
  s2: "e.g. Lead went cold after Day 1 DM. High-ticket offer $3k. They said 'I need to think about it.'",
  s3: "e.g. Focus on 3 competitors: Alex Hormozi, Iman Gadzhi, Hamza. Pull hooks and offer angles.",
  s4: "e.g. New client: Sarah. She's a life coach doing $8k/mo. Wants to scale with content + automation.",
  s5: "e.g. Building a quiz funnel for our AI agency offer. Stack: Typeform + Make + Notion + Stripe.",
  s6: "e.g. Top objection this month: 'I don't have time.' Need 3 content angles to flip this into a hook.",
  s7: "e.g. Week of March 17. Completed: sales copy rewrite. In progress: onboarding flow, Supabase setup.",
...
function OrgChart({ cfg, setActive, setChatAgent, tasks }) {
  ...
  const goToChat = (id) => { setChatAgent(id); setActive("chat"); };
...
function Pipeline() {
  ...
  const [addingTo, setAddingTo] = useState(null); // stage label
